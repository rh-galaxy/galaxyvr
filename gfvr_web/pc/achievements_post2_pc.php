	...
	if(checkstring($paramArray) && $_POST["LEVEL"]!="" && $_POST["NAME"]!="" && $_POST["USERID"]!="" && $_POST["COUNTER"]!="" && $_POST["SCORE"]!="" && $_POST["REPLAY"]!="") {
		...
		if($isok) {
			...
			if($num_rows==1) {
				...
				$query = "SELECT achievements2_t.score FROM achievements2_t WHERE level='".$_POST["LEVEL"]."' AND user_id='".$userid."'";
				...
				if($qualify) {
					...
					if($num_rows!=0) {
						$query = "UPDATE achievements2_t SET ip='".$ip."',timestamp=NOW(),score='".$_POST["SCORE"]."',replay='".$replay."' WHERE (level='".$_POST["LEVEL"]."' AND user_id='".$userid."')";
					} else {
						$query = "INSERT INTO achievements2_t (ip, user_id, level, score, replay, steam) VALUES('".$ip."','".$userid."','".$_POST["LEVEL"]."',".$_POST["SCORE"].",'".$replay."',".$steam.")";
					}
