			...
			if($num_rows > 0) {
				...
				for ($j=0; $j < floor(($num_rows+($num_per_table-1))/$num_per_table); $j++) {
					...
					for($i=$j*$num_per_table; $i < $num_rows; $i++) {
						...
						echo "<td><a href=\"print_hiscore_for_user_pc.php?Name=".$row["name"]."&Id=".$row["id"]."\">";
						if(strlen($row["name"])>26) echo substr($row["name"], 0, 24)."..";
						else echo $row["name"];
						echo "</a></td>";
						
						echo "<td>";
						$remain = $row["score_sum"];
		...
		if($result) {
			...
			if($num_rows > 0) {
				for($i=0; $i < $num_rows; $i++) {
					...
					echo "<tr><td><i>";
					echo "<a href=\"print_hiscore_for_level_pc.php?Level=".$row["Level"]."\">";
					echo $row["Level"];
					echo "</a>";
					echo "</i></td>";
